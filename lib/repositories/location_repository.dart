    ...
    try {
      ...
      if (!serviceEnabled) {
        return LocationResult.error(
            '$kLocationServicesDisabled. Please enable them in settings.');
      }
      ...
      if (permission != LocationPermissionState.granted) {
        switch (permission) {
          case LocationPermissionState.denied:
            return LocationResult.error(
                'Location access was denied. To use automatic location detection, please enable location permissions for this app in your device settings.');
          case LocationPermissionState.deniedForever:
            return LocationResult.error(
                'Location access is permanently disabled. Please go to your device settings > Apps > Simple Azaan > Permissions and enable Location.');
          case LocationPermissionState.notDetermined:
            return LocationResult.error(
                'Location permission could not be determined. Please check your device location settings.');
          default:
            return LocationResult.error(
                'Location access is required for automatic location detection.');
        }
      }
      ...
      if (placemarks.isEmpty) {
        return LocationResult.error(
            'Unable to determine address from location.');
      }
      ...
      if (e is LocationServiceDisabledException) {
        ...
        return LocationResult.error(
            '$kLocationDetectionTimeout. Please try again.');
      } else {
        return LocationResult.error(
            '$kLocationDetectionFailed: ${e.toString()}');
      }
    }
  }
